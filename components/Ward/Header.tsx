...
const Header = () => {
  ...
  const uploadMedia = async (docRefId: string) => {
    ...
    await updateDoc(doc(db, "ward", userData.ward, docRefId), {
  ...
  const sendPost = async () => {
    ...
    try {
      const docRef = await addDoc(collection(db, "ward", userData.ward), {
        id: user.id,
        text: input.trim(),
  ...
  return (
    <View className="shadow-md p-4">
      <View className="flex-row items-center justify-between">
        <Text className="font-extrabold text-3xl">{userData.ward}</Text>
        <Image
