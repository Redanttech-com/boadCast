...
const Posts = ({ post, id, openBottomSheet }) => {
  ...
  useEffect(() => {
    try {
      ...
      const unsubscribe = onSnapshot(
        collection(db, "ward", userData.ward, id, "likes"),
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      collection(db, "ward", userData.ward, id, "comments"),
    ...
    try {
      ...
      if (userData && userData.id && id) {
        if (hasLiked) {
          // Unlike the post
          await deleteDoc(
            doc(db, "ward", userData.ward, id, "likes", userData.id)
          );
        } else {
          // Like the post
          await setDoc(
    ...
    const fetchPost = async () => {
      try {
        ...
        if (userSnap.exists()) {
          ...
          const postRef = doc(db, "ward", userData.ward, id);
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              await deleteDoc(doc(db, "ward", userData.ward, id));
              ...
              const imageRef = ref(storage, `ward/${id}/image`);
              try {
  ...
  const cite = async () => {
    ...
    if (post && user) {
      ...
      if (
        ...
        try {
          await addDoc(collection(db, "ward", userData.ward), {
            id: user.id,
            text: postData.text,
            citeInput: citeInput,
            userImg: userData.userImg,
