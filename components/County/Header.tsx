...
const Header = () => {
  ...
  const uploadMedia = async (docRefId: string) => {
    ...
    await updateDoc(doc(db, "county", userData.county, docRefId), {
  ...
  const sendPost = async () => {
    ...
    try {
      const docRef = await addDoc(collection(db, "county", userData.county), {
        id: user.id,
        text: input.trim(),
    ...
    <View className="shadow-md p-4">
      ...
      {media.uri && (
        <Image
          ...
          className="h-40 w-full mt-4 rounded-lg animate-bounce"
        />
      )}
